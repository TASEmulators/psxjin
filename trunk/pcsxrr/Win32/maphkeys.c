...
//	char* szName = "";
//
//	for (int i = 0; RealKeyNames[i].nCode; i++) {
...
//	for (unsigned int i = 0; i < NUM_EMU_CMDS; i++) {
//		LVITEM LvItem;
//
...
//	}
//
//	DestroyWindow(hHkeyssDlg);							// Make sure any existing dialogs are gone
//	DestroyWindow(hHkeyssDlg);							//
//
//
//	nHkeyssInput = nSel;
//	HkeyssCreate();
...
//
//	hMHkeysDlg = FBACreateDialog(hAppInst, MAKEINTRESOURCE(IDD_MHKEYS), hScrnWnd, DialogProc);
//	if (hMHkeysDlg == NULL) {
//		return 1;
//	}
//
//	WndInMid(hMHkeysDlg, hScrnWnd);
