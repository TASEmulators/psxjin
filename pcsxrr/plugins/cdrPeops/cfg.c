...
void EnumDrives(HWND hW)
{
 ...
 tempDest=ComboBox_AddString(hWC,"NONE");                       // add always existing 'none'
 ...
 for(i=0;i<iNum;i++)                                   // loop drives
  {
   tempDest=ComboBox_AddString(hWC,pN);                         // -> add drive name
   p=strchr(pN,']');
   if(p)
    {
     *p=0;
 ...
 tempDest=ComboBox_SetCurSel(hWC,k);                            // do the drive sel
...
void GetCDRInfos(HWND hW,int * iA, int * iT,int * iL)
{
 ...
 tempDest=ComboBox_GetLBText(hWC,i,szB);                        // get cd text
 p=szB+1;
...
void OnIMode(HWND hW)
{
 ...
 tempDest=ComboBox_SetCurSel(hWC,iInterfaceMode);               // sel interface again (maybe it was not supported on open)
...
void ShowSubFileStuff(HWND hW)
{
 ...
 if(iSel==1)
  {
   tempDest=ComboBox_SetCurSel(GetDlgItem(hW,IDC_CACHE),0);
   ShowWindow(GetDlgItem(hW,IDC_DATACACHE),SW_HIDE);
  }
}
...
BOOL OnInitCDRDialog(HWND hW) 
{
 ...
 tempDest=ComboBox_AddString(hWC,"NONE");
 tempDest=ComboBox_AddString(hWC,"W9X/ME - ASPI scsi commands");
 tempDest=ComboBox_AddString(hWC,"W2K/XP - IOCTL scsi commands");
 tempDest=ComboBox_AddString(hWC,"W2K/XP - IOCTL raw reading");
 tempDest=ComboBox_SetCurSel(hWC,iInterfaceMode);

 hWC=GetDlgItem(hW,IDC_RTYPE);                         // read mode
 tempDest=ComboBox_AddString(hWC,"NONE");
 tempDest=ComboBox_AddString(hWC,"BE_1 (ATAPI SPEC 1)");
 tempDest=ComboBox_AddString(hWC,"BE_2 (ATAPI SPEC 2)");
 tempDest=ComboBox_AddString(hWC,"28_1 (SCSI READ10)");
 tempDest=ComboBox_AddString(hWC,"28_2 (SCSI READ10 SP)");
 tempDest=ComboBox_AddString(hWC,"D8_1 (SPECIAL)");
 tempDest=ComboBox_AddString(hWC,"D4_1 (SPECIAL)");
 tempDest=ComboBox_AddString(hWC,"D4_2 (SPECIAL)");
 tempDest=ComboBox_SetCurSel(hWC,iRType);

 EnumDrives(hW);                                       // enum drives

 hWC=GetDlgItem(hW,IDC_CACHE);                         // caching
 tempDest=ComboBox_AddString(hWC,"None - reads one sector");
 tempDest=ComboBox_AddString(hWC,"Read ahead - fast, reads more sectors at once");
 tempDest=ComboBox_AddString(hWC,"Async read - faster, additional asynchronous reads");
 tempDest=ComboBox_AddString(hWC,"Thread read - fast with IOCTL, always async reads");
 tempDest=ComboBox_AddString(hWC,"Smooth read - for cd drives with psx cd reading troubles");
 tempDest=ComboBox_SetCurSel(hWC,iUseCaching);
 ...
 tempDest=ComboBox_AddString(hWC,"Don't read subchannels (certain copy-protected games will not work)");
 tempDest=ComboBox_AddString(hWC,"Read subchannels (slow, few drives support it, best chances with BE mode)");
 tempDest=ComboBox_AddString(hWC,"Use subchannel SBI/M3S info file (recommended)");
 tempDest=ComboBox_SetCurSel(hWC,iUseSubReading);

 ShowSubFileStuff(hW);                                 // show/hide subc controls

 hWC=GetDlgItem(hW,IDC_SPEED);                         // speed limit
 tempDest=ComboBox_AddString(hWC,"2 X");
 tempDest=ComboBox_AddString(hWC,"4 X");
 tempDest=ComboBox_AddString(hWC,"8 X");
 tempDest=ComboBox_AddString(hWC,"16 X");
...
void OnCDRAuto(HWND hW) 
{
 ...
 if(OpenGenCD(iA,iT,iL,1)>0)                           // open cd
  {
   ...
   if(iFoundType<=0)                                   // no mode
    ...
    {                                                  // or show the detected one
     ...
     tempDest=ComboBox_GetLBText(hWC,iFoundType,szB+lstrlen(szB));
    }
 ...
 tempDest=ComboBox_SetCurSel(hWC,iFoundType);
}
...
{
 unsigned char * pB, * pX;
 int iA,iT,iL,iSSpeed=1,iFoundType;
   ...
   case WM_INITDIALOG:
    {
     ...
     tempDest=ComboBox_AddString(hWC,"Read subchannels from CD");
     tempDest=ComboBox_AddString(hWC,"Convert a CloneCD SUB file");
     ...
     if(i!=CB_ERR) 
      {
       char szB[256];
       tempDest=ComboBox_GetLBText(hWC,i,szB);
